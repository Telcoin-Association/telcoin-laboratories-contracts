...
contract V4HookMinerDeployer is Script {
    ...
    bytes32 constant POLYGON_WETH_TEL_POOLID = bytes32(0x9a005a0c12cc2ef01b34e9a7f3fb91a0e6304d377b5479bd3f08f8c29cdf5deb);
    bytes32 constant POLYGON_USDC_EMXN_POOLID = bytes32(0xfd56605f7f4620ab44dfc0860d70b9bd1d1f648a5a74558491b39e816a10b99a);
    ...
    function _deployContracts(ChainConfig storage config) internal {
        ...
        bytes memory subscriberConstructorArgs = abi.encode(
    ...
    function _initializePool(ChainConfig storage config, PoolConfig storage poolConfig, PoolKey memory key, uint160 sqrtPriceX96) internal {
    ...
    function _postDeploymentChecks(ChainConfig storage chainConfig, PoolConfig storage  poolConfig, PoolKey memory key) internal view {
        ...
        require(positionRegistry.hasRole(positionRegistry.UNI_HOOK_ROLE(), address(telxIncentiveHook)), "!UNI_HOOK_ROLE");
        require(positionRegistry.hasRole(positionRegistry.SUPPORT_ROLE(), chainConfig.supportSafe), "!SUPPORT_ROLE");
        require(positionRegistry.hasRole(positionRegistry.SUBSCRIBER_ROLE(), address(telxSubscriber)), "!SUBSCRIBER_ROLE");
        ...
        (Currency currency0, Currency currency1, uint24 fee, int24 tickSpacing, IHooks hooks) = positionRegistry.initializedPoolKeys(key.toId());
