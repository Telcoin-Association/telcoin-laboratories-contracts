...
**Design rule.** No removal from the index decides on anything a third party can move. Entries leave on pool allowlisting, position ownership, the position's own liquidity, or an admin call; the pool's aggregate liquidity and its current tick NEVER decide a removal, because both can be set to anything inside a single `PoolManager.unlock` for the cost of gas. The one write that reads the tick is the in-range gate at subscribe time, which can only refuse (the LP retries) and runs under the unlock guard, so holding a price against it costs a real swap and gains nothing. Any future write that reads pool state to remove an entry is a regression of this rule and needs the same adversarial fork test that `test_pruneSubscription_refusedInsideUnlock` provides.

- `validPool(poolId)` means "on the admin allowlist AND initialized on chain". Allowlisting is the TELx decision; initialization is the Uniswap fact. `handleSubscribe` requires both. The allowlist is populated by `registerPool(PoolKey)` in the same Safe batch that deploys the registry, so it is never empty on a live registry.
- **Every allowlisted pool MUST carry a non-zero `minLiquidity` floor.** The allowlist alone does not make a cap slot cost capital: with a zero floor, a full-range position of liquidity 1 in a real TELx pool costs a wei of each token and holds a slot (confirmed on a Polygon fork). The floor is sized so that the narrowest in-range position, one tick spacing wide, is worth `pools.json`'s `minPositionValue1` of currency1 at the opening price (`V4PoolMath.minLiquidityForNarrowestPosition`); wider positions need proportionally more (about 33x for a +/-10% band, 670x for the full range at spacing 60). The deploy batch sets it with `setMinLiquidity` and refuses to go out while it is undecided; `VerifyTELxRegistry` asserts it from the same derivation. The floor assumes `inRangeRequired` is on: a band far below the price holds the same liquidity for far less, so turning the gate off reopens gas-only slots unless the floors are raised.
- A slot is held only while the position keeps its liquidity. A drained position is prunable by anyone, so capital cannot be withdrawn and reused for the next slot; holding N slots at once means locking N floors' worth.
...
- Subscription threshold: a position needs non-zero liquidity and at least the pool's admin-set absolute `minLiquidity`. There is no relative-to-pool threshold: a gate expressed as a fraction of the pool's liquidity is movable by anyone inside an unlock, and the Snapshot strategy already values positions in USD, so dust votes as dust. The floor's job is the cap, not vote weight.
...
- `resubscribe(tokenId)` is permissionless and MUST index only a position that v4 reports subscribed to a holder of `SUBSCRIBER_ROLE`, under exactly `handleSubscribe`'s checks. It exists because the index can fall behind v4 (a drained position pruned and then refilled; a stale entry after a swallowed unsubscribe) and v4 refuses a second `subscribe`; without it the owner's only recovery is unsubscribe then subscribe, and nothing tells them to. Off-chain tooling that notices `positionManager.subscriber(tokenId)` set with `isTokenSubscribed` false SHOULD call it.
...
- `handleSubscribe` MUST be idempotent for the same owner: a tokenId already indexed under its current owner is a no-op. One indexed under a previous owner (a swallowed unsubscribe followed by a transfer) MUST be moved to the current owner, never silently dropped: v4 has already accepted the new owner's opt-in by the time the registry hears of it.
...
- `MAX_SUBSCRIBED = 50_000` is GLOBAL across all pools, on distinct owners, checked only on an owner's first subscription. `MAX_SUBSCRIPTIONS = 1_000` per address. Subscribe and unsubscribe are O(1) (swap-and-pop); the only linear-in-N path is the `getSubscriptions` view. `forceUnsubscribe(tokenId)` and `forceUnsubscribeBatch(tokenIds)` (`DEFAULT_ADMIN_ROLE`) are the eviction backstop; the batch form exists so a filled cap is one Safe transaction, not fifty thousand.
- `DEFAULT_ADMIN_ROLE` follows `AccessControlDefaultAdminRules`: one holder, moved only by a two-step transfer with a 3 day delay (`ADMIN_TRANSFER_DELAY`), never granted to a second address or renounced in one call. An admin lost by operator error would freeze the allowlist, the floors, the gate and eviction at the CREATE3 address forever.
- The registry index MUST hold under random interleavings of every write path. `test/telx/PositionRegistry.invariant.t.sol` drives subscribe, unsubscribe (including the swallowed case), transfer, burn, drain, refill, prune, resubscribe, eviction, allowlist and floor changes, price moves and the unlock guard, and asserts the swap-and-pop bookkeeping, the global set, both caps, and that the index never holds a token v4 has not subscribed unless a notification was swallowed. Extend the handler when a write path is added.
...
- `getAmountsForLiquidity` MUST return, to the wei, what Uniswap's reference `LiquidityAmounts.getAmountsForLiquidity` returns; the Snapshot strategy was written against it. It is computed from core `SqrtPriceMath` with round-down, and `testFuzz_getAmountsForLiquidity_matchesReference` pins the equivalence.
...
- `setRegistry` MUST refuse a target with no code or one that has not granted this subscriber `SUBSCRIBER_ROLE`, and the constructor MUST refuse a codeless registry too: `try` does not catch the code-existence check that precedes an external call, so a codeless registry would turn the wrapped `notifyBurn` into a burn that always reverts. `renounceOwnership` MUST revert. The owner is the governance Safe: `setRegistry` repoints every LP subscription and is a governance lever, and governance holding both the registry admin role and the subscriber means it can always recover a misconfiguration.
...
- Everything a chain needs to be live goes out as ONE MultiSend: deploys, role grants, the allowlist registrations and the per-pool liquidity floors. A registry can never exist on chain with its subscriber unwired, its allowlist empty, or a pool whose cap slots cost nothing. The floors are derived from `pools.json` (`PoolsJson.minLiquidityFloor`) by the deploy and re-derived by the verify script, and the deploy refuses to propose while any pool's amounts or `minPositionValue1` are undecided.
