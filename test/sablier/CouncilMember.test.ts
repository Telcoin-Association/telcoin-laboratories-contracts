...
describe("CouncilMember", () => {
  ...
  let stream: TestSablierV2Lockup;

  let target: SignerWithAddress;
  let id: number = 0;
  let governanceRole: string = ethers.keccak256(
    ethers.toUtf8Bytes("GOVERNANCE_COUNCIL_ROLE")
  );
  let supportRole: string = ethers.keccak256(
    ethers.toUtf8Bytes("SUPPORT_ROLE")
  );
  ...
  beforeEach(async () => {
    ...
    const TestTelcoinFactory = await ethers.getContractFactory(
      "TestTelcoin",
      admin
    );
    telcoin = await TestTelcoinFactory.deploy(admin.address);

    const TestSablierV2LockupFactory = await ethers.getContractFactory(
      "TestSablierV2Lockup",
      admin
    );
    stream = await TestSablierV2LockupFactory.deploy(
      await telcoin.getAddress()
    );

    const CouncilMemberFactory = await ethers.getContractFactory(
      "CouncilMember",
      admin
    );
    councilMember = await CouncilMemberFactory.deploy();

    await councilMember.initialize(
      await telcoin.getAddress(),
      "Test Council",
      "TC",
      stream.getAddress(),
  ...
  describe("Values", () => {
    describe("Getters", () => {
      it("GOVERNANCE_COUNCIL_ROLE", async () => {
        expect(await councilMember.GOVERNANCE_COUNCIL_ROLE()).to.equal(
          governanceRole
        );
      });
      ...
      it("TELCOIN address", async () => {
        expect(await councilMember.TELCOIN()).to.equal(
          await telcoin.getAddress()
        );
      });
      ...
      it("has governance role", async () => {
        expect(
          await councilMember.hasRole(governanceRole, admin.address)
        ).to.equal(true);
      });

      it("has support role", async () => {
        expect(
          await councilMember.hasRole(supportRole, support.address)
        ).to.equal(true);
      });
      ...
      it("has AccessControlEnumerableUpgradeable interface", async () => {
        ...
        expect(await councilMember.supportsInterface(interfaceId)).to.equal(
          true
        );
      ...
      describe("updateLockup", () => {
        describe("Failure", () => {
          it("updateLockup should fail when caller does not have role", async () => {
            await expect(
              councilMember.connect(support).updateLockup(support.address)
            ).to.be.reverted;
          });
        });
        ...
        describe("Success", () => {
          it("updateLockup", async () => {
            await expect(councilMember.updateLockup(support.address))
              .emit(councilMember, "LockupUpdated")
              .withArgs(support.address);
      ...
      describe("updateID", () => {
        describe("Failure", () => {
          it("updateID should fail when caller does not have role", async () => {
            await expect(councilMember.connect(support).updateID(1)).to.be
              .reverted;
          });
        });
        ...
        describe("Success", () => {
          it("updateID", async () => {
            await expect(councilMember.updateID(1))
              .emit(councilMember, "IDUpdated")
              .withArgs(1);
  ...
  describe("mutative", () => {
    ...
    describe("mint", () => {
      it("mint a single NFT", async () => {
        await expect(councilMember.mint(member.address)).emit(
          councilMember,
          "Transfer"
        );
    ...
    describe("approve", () => {
      ...
      describe("Failure", () => {
        it("approval is created for and only for designated address", async () => {
          await expect(
            councilMember
              .connect(holder)
              .transferFrom(member.address, support.address, 0)
          ).to.be.reverted;
          expect(await councilMember.balanceOf(support.address)).to.equal(0);
        });
      });
      ...
      describe("Success", () => {
        it("approval is created for and only for designated address", async () => {
          await expect(
            councilMember.connect(admin).approve(support.address, 0)
          ).emit(councilMember, "Approval");
          await expect(
            councilMember
              .connect(support)
              .transferFrom(member.address, support.address, 0)
    ...
    describe("burn", () => {
      beforeEach(async () => {
        ...
        expect(await councilMember.mint(await stream.getAddress())).to.not
          .reverted;
      });
      ...
      describe("Failure", () => {
        it("the correct removal is made", async () => {
          ...
          await expect(councilMember.burn(2, support.address)).to.revertedWith(
            "CouncilMember: must maintain council"
          );
        });
      });
      ...
      describe("Success", () => {
        it("the correct removal is made", async () => {
          await expect(councilMember.burn(1, support.address)).emit(
            councilMember,
            "Transfer"
          );
        });
    ...
    describe("Extended burn", () => {
      beforeEach(async () => {
        ...
        await councilMember.mint(await stream.getAddress());
        await councilMember.mint(member.address);
        await councilMember.mint(support.address);
        await councilMember.mint(await stream.getAddress());
      });
      ...
      it("the correct removal is made", async () => {
        ...
        await expect(councilMember.burn(0, support.address))
          .emit(councilMember, "Transfer")
          .withArgs(
            member.address,
      ...
      it("the correct removal is made", async () => {
        ...
        await expect(councilMember.burn(1, support.address))
          .emit(councilMember, "Transfer")
          .withArgs(
            support.address,
            "0x0000000000000000000000000000000000000000",
      ...
      it("the correct removal is made", async () => {
        expect(await councilMember.totalSupply()).to.equal(6);
        expect(
          await councilMember.balanceOf(await stream.getAddress())
        ).to.equal(2);
        expect(await councilMember.tokenIdToBalanceIndex(5)).to.equal(5);
        expect(await councilMember.balanceIndexToTokenId(5)).to.equal(5);

        await expect(councilMember.burn(2, support.address))
          .emit(councilMember, "Transfer")
          .withArgs(
      ...
      it("the correct removal is made", async () => {
        ...
        await expect(councilMember.burn(3, support.address))
          .emit(councilMember, "Transfer")
          .withArgs(
            member.address,
            "0x0000000000000000000000000000000000000000",
      ...
      it("the correct removal is made", async () => {
        ...
        await expect(councilMember.burn(4, support.address))
          .emit(councilMember, "Transfer")
          .withArgs(
            support.address,
            "0x0000000000000000000000000000000000000000",
      ...
      it("the correct removal is made", async () => {
        ...
        await expect(councilMember.burn(5, support.address))
          .emit(councilMember, "Transfer")
          .withArgs(
            await stream.getAddress(),
            "0x0000000000000000000000000000000000000000",
        ...
        expect(await councilMember.tokenIdToBalanceIndex(5)).to.equal(
    ...
    describe("transferFrom", () => {
      ...
      describe("Success", () => {
        it("the correct removal is made", async () => {
          ...
          expect(await councilMember.balanceOf(support.address)).to.equal(1);
          expect(
            await councilMember.transferFrom(member.address, support.address, 0)
          ).to.not.reverted;
  ...
  describe("tokenomics", () => {
    ...
    describe("mint", () => {
      it("correct balance accumulation", async () => {
        ...
        expect(await telcoin.balanceOf(support.address)).to.equal(0);
        expect(
          await telcoin.balanceOf(await councilMember.getAddress())
        ).to.equal(200);
    ...
    describe("claim", () => {
      it("claiming rewards", async () => {
        ...
        const memberBalance = await councilMember.balances(0);
        await expect(councilMember.connect(member).claim(0, memberBalance)).to
          .not.reverted; // member claims 200 TEL here
        ...
        expect(await councilMember.balances(2)).to.equal(0);
        await expect(councilMember.connect(member).claim(0, 100)).to.not
          .reverted;
        ...
        expect(await telcoin.balanceOf(support.address)).to.equal(0); // has not claimed
        expect(
          await telcoin.balanceOf(await councilMember.getAddress())
        ).to.equal(200);
      });
